...
def solve_schedule(config: ClinicConfig, num_weeks: int = 2, time_limit_seconds: int = 30,
    ...
    # === SOLVER KNOWLEDGE: Bakjourslinje — alltid aktiv, per-läkare config ===
    # Inte en togglebar regel — inbyggd solverkunskap. Aktiveras av Doctor.backup_call_config
    ...
    if bak_eligible:
        ...
        print(f"  Bakjourslinje (inbyggd): {len(bak_eligible)} seniora läkare med config")

    # === SOLVER KNOWLEDGE: Konsultschema — alltid aktiv, per-läkare config ===
    # Inte en togglebar regel — inbyggd solverkunskap. Aktiveras av Doctor.consultation_schedule
    consult_penalty = []
        ...
        for entry in consult_sched:
            ...
            for week in range(num_weeks):
                ...
                if (doc.id, day, target_func) in x:
                    # Mjuk: penalty om inte på konsultfunktion den dagen
    ...
    if consult_count:
        print(f"  Konsultschema (inbyggd): {consult_count} läkare med konsulttider")

    # === SOLVER KNOWLEDGE: Senior/junior-par på OP — alltid aktiv ===
    # Inte en togglebar regel — inbyggd solverkunskap. Aktiveras av Doctor.op_pairing
    ...
    seniors_who_supervise = [d for d in config.doctors if d.role in senior_roles]
    ...
    for op_fid in op_func_ids:
        for day in range(num_days):
            ...
            for junior in juniors_needing_pair:
                ...
                # Hard: junior på OP → minst en senior måste vara på samma OP
    ...
    if juniors_needing_pair:
        print(f"  Senior/junior OP-par (inbyggd): {len(juniors_needing_pair)} juniorer, {len(seniors_who_supervise)} seniorer")
